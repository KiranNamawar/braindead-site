  ...
  match(query: string, target: string): number | null {
    ...
    if (queryWords.length > 1) {
      ...
      if (wordMatches > 0) {
        ...
        return Math.min(0.95, 0.7 * (wordMatches / queryWords.length) + exactMatchBonus);
      }
    }
    ...
    if (queryLower.length <= 2) {
      ...
      const wordPrefixRegex = new RegExp(`\\b${this.escapeRegExp(queryLower)}`, 'i');
    ...
    if (queryLower.length >= 3) {
      ...
      for (const targetWord of targetWords) {
        if (targetWord.length >= 4) { // Only consider substantial words
          const wordDistance = this.levenshteinDistance(queryLower, targetWord);
          if (wordDistance <= maxDistance) {
            const wordDistanceScore = 1 - (wordDistance / (Math.max(queryLower.length, targetWord.length) + 1));
    ...
    const lengthPenalty = Math.min(0.1, queryChars.length / targetLower.length * 0.1);
...
function extractWords(text: string): string[] {
  ...
  return words.filter(word => {
    ...
    const stopWords = new Set([
      'the', 'and', 'for', 'with', 'that', 'this', 'from', 'are', 'not', 'have', 'has'
    ]);
 ...
 */
function generateNGrams(text: string, minSize: number = 3, maxSize: number = 4): string[] {
  const normalized = normalizeText(text);
  const ngrams: string[] = [];
  ...
  for (let size = minSize; size <= Math.min(maxSize, normalized.length); size++) {
  ...
  search(query: string): SearchResult[] {
    ...
    if (queryWords.length > 1) {
      ...
      this.index.utilities.forEach((utility, id) => {
        ...
        // If most words match in description, add as a result with lower score
          ...
          addOrUpdateResult(id, 'description', 0.6 + (descMatchPercentage * 0.15));
        }
      });
    }
    ...
    this.index.utilities.forEach(utility => {
      ...
      if (!existingResult || existingResult.relevanceScore < 0.7) {
        ...
        const nameMatch = this.index.fuzzyMatcher.match(queryLower, utility.name);
        if (nameMatch) {
          addOrUpdateResult(utility.id, 'name_fuzzy', nameMatch * 0.9);
        }
        
        // Try fuzzy matching on description (lower priority)
        const descMatch = this.index.fuzzyMatcher.match(queryLower, utility.description);
        if (descMatch) {
        ...
        utility.keywords.forEach(keyword => {
          const keywordMatch = this.index.fuzzyMatcher.match(queryLower, keyword);
          if (keywordMatch) {
  ...
  getSuggestions(query: string): SearchSuggestion[] {
    ...
    allKeywords.forEach(keyword => {
      ...
      if (normalizedKeyword === queryLower) {
        ...
        const fuzzyScore = this.index.fuzzyMatcher.match(queryLower, normalizedKeyword);
    ...
    utilityRegistry.categories && Object.values(utilityRegistry.categories).forEach(category => {
      ...
      // Fuzzy match gets lower score
        ...
        const fuzzyScore = this.index.fuzzyMatcher.match(queryLower, category.name);
    ...
    for (const utility of recentUtilities) {
      ...
      const nameMatch = this.index.fuzzyMatcher.match(queryLower, normalizeText(utility.name));
      
      // If we have a fuzzy match or the query is very short (likely a typo)
      if ((nameMatch && nameMatch > 0.5) || 
          (queryLower.length <= 4 && utility.name.toLowerCase().includes(queryLower.charAt(0)))) {
        addSuggestion('utility', utility.name, (nameMatch || 0.6) * 0.85, utility);
      }
      
      // Also check keywords for recently used utilities
      utility.keywords.forEach(keyword => {
        const keywordMatch = this.index.fuzzyMatcher.match(queryLower, normalizeText(keyword));
    ...
    if (queryLower.length <= 4 && suggestions.length < 3) {
      utilityRegistry.utilities
        ...
        .forEach(utility => {
          ...
          if (!suggestions.some(s => s.type === 'utility' && s.text === utility.name)) {
